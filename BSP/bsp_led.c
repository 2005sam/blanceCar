 ...
 * @param  led: LED identifier (BSP_LED_VOLTAGE_1, BSP_LED_VOLTAGE_2, BSP_LED_VOLTAGE_3)
...
uint32_t BspLedLightOn(LedId led)
{
  switch (led)
  {
  case BSP_LED_VOLTAGE_1:
    HAL_GPIO_WritePin(BSP_LED_VOLTAGE_1_GPIO_PORT, BSP_LED_VOLTAGE_1_PIN, GPIO_PIN_SET);
    break;
  case BSP_LED_VOLTAGE_2:
    HAL_GPIO_WritePin(BSP_LED_VOLTAGE_2_GPIO_PORT, BSP_LED_VOLTAGE_2_PIN, GPIO_PIN_SET);
    break;
  case BSP_LED_VOLTAGE_3:
    HAL_GPIO_WritePin(BSP_LED_VOLTAGE_3_GPIO_PORT, BSP_LED_VOLTAGE_3_PIN, GPIO_PIN_SET);
    break;
  default:
    return MAKE_RETURN_CODE(RETURN_ERROR, ERROR_LEVEL_MAJOR, MODULE_LED, ERR_TYPE_INVALID_PARAM, 0x01);
  }
  return MAKE_RETURN_CODE(RETURN_SUCCESS, ERROR_LEVEL_MINOR, MODULE_LED, ERR_TYPE_SUCCESS, 0x00);
}
